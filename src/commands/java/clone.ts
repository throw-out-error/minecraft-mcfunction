...
export class CloneCommand extends Command<"clone", Args> {
  ...
  constructor(
    ...
    if (!filterOrMaskMode) {
      this.maskMode = "replace";
    } else if (isMaskMode(filterOrMaskMode)) {
      this.maskMode = filterOrMaskMode;
    } else {
      this.filter = filterOrMaskMode;
    }
  ...
  get [Command.ARGUMENTS]() {
    const args: Argument[] = [this.begin, this.end, this.destination];
...
export function clone(
  ...
  return new CloneCommand(begin, end, destination, filterOrMaskMode, cloneMode);
}
