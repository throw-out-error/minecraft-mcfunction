...
class McFunction {
    ...
    compile(path: string) {
        const functionPath = `${path}/${this.path}.mcfunction`;
    ...
    static copy(funct: McFunction): McFunction {
        const copy = new McFunction("_");
        for (const key of Object.getOwnPropertyNames(funct))
            copy[key] = funct[key];
        return copy;
    }
}
...
class Command {
    ...
    compile(): string {
        return `${this.method} ${this.params
            .map((p) =>
                p instanceof Value || p instanceof Selector ? p.compile() : p
            )
            .join(" ")}`;
    }
}
