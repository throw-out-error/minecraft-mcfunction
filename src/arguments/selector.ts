...
interface SelectorArguments {
    ...
    level?: Range;
    gamemode?: GameMode | Partial<Record<GameMode, boolean>>;
    name?: {
        [name: string]: boolean;
    };
    x_rotation?: Range;
...
export class Selector extends ArgumentObject {
        ...
        args.forEach(([arg, val]) => {
            switch (arg) {
                ...
                case "advancements": {
                    ...
                    const advancements = Object.entries<Adv>(val).map(
                        ([adv, v]) => {
                            if (typeof v === "boolean")
                                return `${adv}=${v ? "true" : "false"}`;

                            const criterias = Object.entries<boolean>(v).map(
                                ([c, v]) => {
        ...
        for (const [arg, val] of Object.entries(this.arguments))
            switch (arg) {
                ...
                case "predicate":
                    ...
                    argList.push(
                        ...boolanMapToKVPairs(val, arg).map(
                            (p) => `${p[0]}=${p[1]}`
                        )
                    );
                    break;
    ...
    static executor(args: Omit<SelectorArguments, "sort" | "limit"> = {}) {
